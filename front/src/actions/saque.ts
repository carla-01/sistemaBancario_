import { formatCurrency, parseMoney, withdraw } from '../api';
...
export const runSaqueAction = async (form: HTMLElement, deps: ActionDeps) => {
  ...
  try {
    const response = await withdraw({ conta: accountNumber, valor: value });
    if (valueInput) valueInput.value = '';
    deps.showPopout(`Saque realizado com sucesso<br>Valor: ${formatCurrency(value)}<br>Conta: ${response.conta ?? accountNumber}`);
    deps.setFormMessage(form, '');
  } catch (error) {
    deps.setFormMessage(form, error instanceof Error ? error.message : 'Erro ao realizar saque');
  }
};
